...
> **総ストーリー数**: 14（予選MVP Must: 6、予選Should: 3、決勝: 5）
...
#### 受入基準
1. **Given** チャット連携済みのユーザーが **When** Planページを開く **Then** チャンネル一覧に「ミュート推奨（神怠惰）」「通常監視」「要注意（罪怠惰リスク）」のラベルが表示される
2. **Given** 過去30日で自分へのメンションが0回のチャンネルが **When** 判定される **Then** 「ミュート推奨」と表示され、理由「過去30日であなたは呼ばれていません」が説明される
3. **Given** 自分の発言に対するリアクション・返信が平均0.5件以下（3回以上）のチャンネルが **When** 判定される **Then** 「偽勤勉」と表示され、理由「発言しても誰も反応しません。この発言は不要です」が説明される
4. **Given** 報告系・情報共有目的の投稿で反応がない場合 **When** 判定される **Then** 「正常」と判定され、「報告は受領されれば十分。反応がなくても問題ありません」と説明される
5. **Given** 時間外（18:00以降）の発言がある **When** 判定される **Then** 「偽勤勉」と表示され、理由「この時間の対応は翌朝で間に合います」が説明される
...
3. **Given** ミュートしたチャンネルで **When** 24時間ごとにメンションチェックが実行される **Then** メンションなし→「問題なし」継続、メンションあり→「呼ばれてます」通知＋罪怠惰リスク記録
4. **Given** 1週間経過してメンション0回の場合 **When** 最終判定が行われる **Then** 「サボり成功」として確定し、次回の判定精度に反映される
5. **Given** 追跡中にユーザーが「問題あり」を報告した場合 **When** 報告を受け付ける **Then** 罪怠惰として記録され、次回の判定に反映される
...
2. **Given** 判定データがある **When** ダッシュボードを開く **Then** 神怠惰/罪怠惰/偽勤勉の件数・割合が表示される
...
2. **Given** レポートが届いた **When** 内容を確認する **Then** ダメ度スコア、神怠惰/偽勤勉/罪怠惰の件数と具体例、創出時間が表示される
3. **Given** 偽勤勉が検出された **When** レポートに含まれる **Then** 「なぜ不要だったか」の理由と「💡 学び」が表示される
4. **Given** 18:00以降にSlack発言がある **When** レポートに含まれる **Then** 「時間外労働アラート: この時間の対応は全て偽勤勉です」と表示される
...
#### 受入基準
1. **Given** メール連携済みのユーザーが **When** Planページを開く **Then** 未返信メール一覧に「神怠惰（返信不要）」「罪怠惰（要返信）」「偽勤勉（テンプレで十分）」のラベルと理由が表示される
...
**As a** 田中（チームリーダー）
**I want to** チーム全体の偽勤勉コストと時間創出量を一覧で見たい
...
#### 受入基準
1. **Given** チームメンバーが3名以上利用している **When** チームダッシュボードを開く **Then** チーム全体の偽勤勉コスト（時間×人件費単価）が表示される
...
**As a** 中村（バックオフィス）
**I want to** 全社の偽勤勉コストレポートを経営会議に提出したい
...
#### 受入基準
1. **Given** 複数チームのデータがある **When** KPIレポートを生成する **Then** 全社の偽勤勉コスト、時間創出量、改善トレンドがレポートに含まれる
